...
export class CardValidatorWidget {
  ...
  bindToDOM() {
    ...
    this._cards = this._element.querySelectorAll(CardValidatorWidget.cardSelector);
    this._form = this._element.querySelector(CardValidatorWidget.formSelector);
    this._input = this._element.querySelector(CardValidatorWidget.inputSelector);
    this._submitter = this._element.querySelector(CardValidatorWidget.submitterSelector);
    this._message = this._element.querySelector(CardValidatorWidget.messageSelector);
  ...
  getPaySystem(value) {
    ...
    const paySystem = structuredClone(paySystems.find((paySystem) => {
      const paySystemStarts = paySystem.info.map(info => info.start);

      return paySystemStarts.includes(start_1) || paySystemStarts.includes(start_2);
    }));
  ...
  static get cardSelector() { return '.card-validator-widget__card'; }

  static get formSelector() { return '.card-validator-widget__form'; }

  static get inputSelector() { return '.card-validator-widget__input'; }
  ...
  static get messageSelector() { return '.card-validator-widget__message'; }

  static get selector() { return '.card-validator-widget'; }

  static get submitterSelector() { return '.card-validator-widget__btn'; }
}
